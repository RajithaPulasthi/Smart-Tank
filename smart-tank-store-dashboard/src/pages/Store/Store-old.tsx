...
const StorePage = () => {
  ...
  useEffect(() => {
    ...
    const currentStoreData = localStorage.getItem("currentStore");

    let aquariumId = 0;
    if (currentStoreData) {
      try {
        const storeData = JSON.parse(currentStoreData);
        if (storeData && storeData.length > 0 && storeData[0].id) {
          aquariumId = storeData[0].id;
        }
      } catch (e) {
    ...
    if (currentUserData) {
      ...
      // If only store data is available
      setFormData((prev) => ({
        ...prev,
        aquariumId: aquariumId,
      }));
  ...
  const handleSave = async () => {
    ...
    // The aquariumId is already in formData from the useEffect hook
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          enqueueSnackbar("Store information saved successfully!", {
            variant: "success",
          });
          console.log("Store data saved:", responseData);
        } catch (jsonError) {
        ...
        const errorText = await response.text(); // Use text() to avoid JSON parse error if response is not JSON
        enqueueSnackbar(`Error saving store data: ${errorText}`, {
          variant: "error",
        });
        console.error("Error saving store data:", errorText);
      }
    } catch (error) {
      enqueueSnackbar("A network error occurred. Please try again.", {
        variant: "error",
      });
