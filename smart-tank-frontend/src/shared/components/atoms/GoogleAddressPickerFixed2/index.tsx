    ...
    const initializeAutocomplete = () => {
      try {
        ...
        if (!window.google?.maps?.places?.Autocomplete) {
          ...
          setError(
            "Google Maps Places API not available - ensure API key has Places API enabled"
          );
          setIsLoading(false);
          return;
        }
        ...
        autocompleteRef.current.addListener("place_changed", () => {
          ...
          if (!place.geometry.location) {
            console.log(
              "Place geometry has no location:",
              place.name || place.formatted_address
            );
            return;
          }
          ...
          if (!place.formatted_address) {
            console.warn(
              "Place missing formatted_address, using name as fallback"
            );
          }
          ...
          const locationData: LocationData = {
            ...
            address:
              place.formatted_address || place.name || "Address not available",
            city:
              addressComponents.locality ||
              addressComponents.administrative_area_level_2,
          ...
          if (!locationData.city && !locationData.province) {
            console.warn(
              "Location may be outside Sri Lanka or missing administrative data"
            );
          }
        ...
        setError(
          `Failed to initialize address picker: ${
  ...
  return (
    <Paper elevation={1} sx={{ p: 3 }} ref={containerRef}>
      <Box>
        ...
        >
          🌍 Search restricted to Sri Lanka. Start typing to find your store
          location.
        </Typography>
      </Box>
    </Paper>
